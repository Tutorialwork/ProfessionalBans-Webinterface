...
<?php
    ...
    $stmt = MySQLWrapper()->prepare("DELETE FROM accounts WHERE USERNAME = :user");
...
if (isset($_POST["submit"]) && isset($_SESSION["CSRF"])) {
  if (!empty($_POST["username"]) && !empty($_POST["mcusername"])) {
    if ($_POST["CSRFToken"] != $_SESSION["CSRF"]) {
      ...
      $stmt = MySQLWrapper()->prepare("SELECT * FROM bans WHERE NAME = :request");
      $stmt->execute(array(":request" => $_POST["mcusername"]));
      $row = $stmt->fetch();
      if (isPlayerExists($row["UUID"])) {
        $stmt = MySQLWrapper()->prepare("SELECT * FROM accounts WHERE UUID = :request");
        $stmt->execute(array(":request" => $row["UUID"]));
        if ($stmt->rowCount() == 0) {
          $stmt = MySQLWrapper()->prepare("SELECT * FROM accounts WHERE USERNAME = :request");
          $stmt->execute(array(":request" => $_POST["username"]));
          if ($stmt->rowCount() == 0) {
            $stmt = MySQLWrapper()->prepare("INSERT INTO accounts(UUID, USERNAME, PASSWORD, RANK, GOOGLE_AUTH, AUTHCODE) 
...
<div class="flex-container animated fadeIn">
  <div class="flex item-1">
    ...
    <table>
      ...
      $stmt = MySQLWrapper()->prepare("SELECT * FROM accounts");
      $stmt->execute();
